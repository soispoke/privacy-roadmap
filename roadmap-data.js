...
(function () {
  ...
  const ITEMS = [
    ...
    { lane:"crypto", id:"leanvm-frozen", title:"Final leanSTARK design", link:"https://github.com/leanEthereum/leanVM", kind:"headliner", laneSpan:true, oq:["Separate leanSTARK (the proving system + verifier) from the leanVM/leanISA VM in name and spec","VM-based or circuit-based enshrined verifier (Binius64 / Plonky3, AI-assisted circuits)?","Which proving system and verifier do we enshrine?"], track:{"q1-27":"prototype","q2-27":"pfi","q3-27":"sfi","q4-27":"sfi","q1-28":"mainnet"}, sum:"Converges the cryptography explorations into the object Ethereum actually freezes at Q1 2027: leanSTARK, the proving system and the verifier enshrined for it. The leanVM/leanISA VM is an optional developer frontend, not the enshrined object; circuits, Noir, or a custom prover can recurse into the same leanSTARK verifier. Selection criterion is recursion efficiency. leanSTARK rides the familiar STARK brand, with the caveat that the system is actually multilinear. The broad RISC-V / evm-asm programming surface is a separate, later decision." },
    ...
    // e2e — Integration
    { lane:"e2e", id:"e2e-proto", title:"End-to-end private tx prototype", link:"", kind:"headliner", track:{"q3-26":"research","q4-26":"prototype","q1-27":"prototype","q2-27":"devnet","q3-27":"devnet","q4-27":"devnet"}, oq:["Which privacy protocol and wallet drive the first integration (e.g. Railgun via Kohaku)?","Minimal devnet that exercises 8141 + 8250 + 8272 + FOCIL end to end?"], sum:"An end-to-end prototype of a private transaction across the whole stack: wallet, public mempool, privacy protocol (e.g. Railgun), and the L1 protocol, exercising the new EIPs together (8141 frame transactions, 8250 keyed nonces, 8272 recent roots, 7805 FOCIL eligibility, VOPS). Goal: show the pieces compose into a working private send and receive, and surface integration gaps early. Matures from research to a devnet demo as the EIPs land." },
  ];
