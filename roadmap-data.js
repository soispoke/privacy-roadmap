...
(function () {
  ...
  const ITEMS = [
    ...
    { lane:"tput", id:"stark-mempool", title:"Recursive STARK Mempool", link:"https://github.com/ethereum/EIPs/pull/11772", kind:"soft", track:{"q4-27":"sfi","q1-28":"mainnet","q2-26":"eip","q3-26":"prototype","q4-26":"prototype","q1-27":"devnet","q2-27":"pfi","q3-27":"sfi"}, sum:"Sharded mempool plus recursive aggregation to cut gossip bandwidth. EIP-8288 (PR #11772) is the dependency tuple that gets aggregated: (scheme, data_hash, verification_key). Needs the recursion-capable proving system and its enshrined verifier, not a chosen ISA or VM: the aggregation is a fixed set of circuits plus the verifier. Not gated on the RISC-V / evm-asm surface. Aggregation needs a fallback so it never becomes an inclusion gate." },
    ...
    // e2e — Integration
    { lane:"e2e", id:"e2e-proto", title:"End-to-end private tx flow", link:"", kind:"headliner", track:{"q3-26":"research","q4-26":"prototype","q1-27":"prototype","q2-27":"devnet","q3-27":"devnet","q4-27":"devnet"}, oq:["Which privacy protocol and wallet drive the first integration (e.g. Railgun via Kohaku)?","Can the same transaction be FOCIL-eligible under AA VOPS/Profile 2, so omission is objectively checkable if an includer lists it?"], sum:"One private transaction goes from a wallet through the public mempool and a privacy application to the Ethereum protocol, on a devnet running all four EIPs: 8141 frame transactions, 8250 keyed nonces, 8272 recent roots, and 7805 FOCIL. The transaction should benefit from FOCIL inclusion guarantees by fitting AA VOPS/Profile 2, and should use a trustless paymaster enabled by EIP-8141. Goal: show the pieces compose into a working private send and receive, and surface integration gaps early." },
  ];
